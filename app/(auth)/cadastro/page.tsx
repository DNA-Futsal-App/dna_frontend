...
export default function RegisterPage() {
  ...
  const eventId = selectedCategory?.eventId ?? null;
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      router.push(
        `/verificar-email?login=${encodeURIComponent(String(form.get("email")))}`,
      );
    } catch (err) {
      setError(
  ...
  return (
    <AuthShell
      eyebrow="Sua torcida começa aqui"
      title="Crie seu DNA."
      description="Leva menos de dois minutos. A escolha do time é opcional e pode ser alterada depois."
    >
      <form onSubmit={submit} className="grid gap-5">
        <fieldset className="grid gap-4">
          <legend className="mb-3 text-xs font-black uppercase tracking-[.15em] text-cyan">
            Seus dados
          </legend>
        ...
        <fieldset className="grid gap-4 border-t border-white/8 pt-5">
          <legend className="mb-3 flex items-center gap-2 text-xs font-black uppercase tracking-[.15em] text-amber">
            Preferências{" "}
          ...
          <label className="grid gap-1.5 text-sm font-bold">
            Time
            <select className="field" name="teamId" disabled={!eventId}>
              <option value="">Escolha depois</option>
              {teams.map((team) => (
